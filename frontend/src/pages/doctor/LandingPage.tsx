...
export default function DoctorLandingPage() {
  ...
  // TEMP local state for demo. Replace with data from backend.
  ...
  const statusBadge = useMemo(() => {
    if (verificationStatus === "verified")
      return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Verified</span>;
    ...
    return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-800">Rejected</span>;
  }, [verificationStatus]);
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        {verificationStatus === "pending" && (
          <section className="grid lg:grid-cols-2 gap-6">
            <Card className="border-0 bg-white/80 backdrop-blur rounded-2xl shadow-[0_10px_25px_rgba(16,24,40,0.06)]">
              <CardContent className="p-6">
                ...
                <p className="text-[#6B7280]">
                  Thanks for submitting your credentials. Our team will verify your document shortly.
            ...
            <Card className="border-0 bg-white/80 backdrop-blur rounded-2xl shadow-[0_10px_25px_rgba(16,24,40,0.06)]">
              <CardContent className="p-6">
                <h3 className="font-semibold mb-2">Replace Certificate</h3>
                <p className="text-[#6B7280] mb-4 text-sm">
                  If there was a mistake, you can upload a new PDF to replace the current one.
                </p>
                ...
                {certificateFilename && (
                  <p className="mt-3 text-sm text-[#374151]">Selected: {certificateFilename}</p>
                )}
          ...
          <section className="grid lg:grid-cols-2 gap-6">
            <Card className="border-0 bg-white/80 backdrop-blur rounded-2xl shadow-[0_10px_25px_rgba(16,24,40,0.06)]">
              <CardContent className="p-6">
                ...
                <p className="text-[#6B7280]">
                  Your verification was not approved. Please review the reasons below and resubmit your certificate.
                </p>
            ...
            <Card className="border-0 bg-white/80 backdrop-blur rounded-2xl shadow-[0_10px_25px_rgba(16,24,40,0.06)]">
              <CardContent className="p-6">
                <h3 className="font-semibold mb-2">Resubmit Certificate (PDF)</h3>
                <label className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-dashed border-[#E5E7EB] bg-white cursor-pointer hover:bg-white/90">
                  <Upload className="w-4 h-4" />
                  <span className="text-sm">Choose PDF</span>
                  <input
        ...
        {verificationStatus === "verified" && (
          <section className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button variant="outline" className="border-[#E5E7EB] bg-white hover:bg-white/90" onClick={() => navigate("/doctor/availability")}>
                Edit Availability
              </Button>
              <Button variant="outline" className="border-[#E5E7EB] bg-white hover:bg-white/90" onClick={() => navigate("/doctor/profile")}>
                Update Profile
              </Button>
            </div>
          </section>
        ...
        {/* If not verified: Onboarding card (only when pending with no file uploaded yet) */}
        {verificationStatus !== "verified" && (
...
/* Small tile component for verified dashboard */
function Tile({
  icon,
  title,
  value,
