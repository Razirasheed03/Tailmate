...
class DoctorPublicRepository {
    ...
    listVerifiedWithNextSlot(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const rules = yield DoctorWeeklyRule.find({ userId: { $in: ids } }).lean().exec();
            const byUid = new Map();
            ...
            const enriched = yield Promise.all(items.map((doc) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (let i = 0; i <= horizonDays && !nextSlot; i++) {
                    ...
                    if (!rule || !rule.enabled || !Array.isArray(rule.fixtures) || !rule.fixtures.length)
                        continue;
                    const times = Array.from(new Set(rule.fixtures.map(f => String(f.time)).filter(t => /^\d{2}:\d{2}$/.test(t)))).sort((a, b) => a.localeCompare(b));
                    if (times.length) {
    ...
    getDoctorPublicById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!mongoose_1.Types.ObjectId.isValid(id))
                return null;
            const pipeline = [
                { $match: { userId: new mongoose_1.Types.ObjectId(id), "verification.status": "verified" } },
                {
                    $project: {
                        doctorId: "$userId",
                        displayName: "$profile.displayName",
            ...
            const rules = yield DoctorWeeklyRule.find({ userId: new mongoose_1.Types.ObjectId(id) }).lean().exec();
