...
export class DoctorService {
  ...
  } // role guard remains in service [web:507]
  ...
  private toMinutes(hhmm: string) { const [h, m] = hhmm.split(":").map(Number); return h * 60 + m; } // unchanged [web:507]
  private overlaps(aStart: number, aDur: number, bStart: number, bDur: number) { const aEnd = aStart + aDur; const bEnd = bStart + bDur; return aStart < bEnd && aEnd > bStart; } // unchanged [web:507]
  private ymd(d: Date) { const y = d.getFullYear(), m = String(d.getMonth() + 1).padStart(2, "0"), dd = String(d.getDate()).padStart(2, "0"); return `${y}-${m}-${dd}`; } // unchanged [web:507]
  private within4Days(date: string) { const today = new Date(); const min = this.ymd(today); const maxD = new Date(today); maxD.setDate(today.getDate() + 3); const max = this.ymd(maxD); return date >= min && date <= max; } // unchanged [web:507]
  ...
  // ===== NEW: delegate to repository for sessions =====
  ...
  } // service orchestration only [web:507]
